...
__global__ void matrixMulTiled(float *A, float *B, float *C, int n, int tileWidth, int loadWidth) {
    ...
    // Use a smaller width for loading (loadWidth)
    if (tx < loadWidth && ty < loadWidth) {
...
int main() {
    ...
    for (int tileWidth = 2; tileWidth <= MAX_TILE_WIDTH; tileWidth *= 2) {
        ...
        matrixMulTiled<<<dimGrid, dimBlock, sharedMemSize>>>(d_A, d_B, d_C, n, tileWidth, tileWidth);
        ...
        printf("Tile Width: %d, Load Width: %d, Time (ms): %f\n", tileWidth, tileWidth, milliseconds);
