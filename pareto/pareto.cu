...
int main()
{
    ...
    for (bool is_A_transpose : {true, false})
    {
        for (bool is_B_transpose : {true, false})
        {
            ...
            // Calculate TFLOPS
            double const tflops_hmma{(((matrix_size_m * matrix_size_n * matrix_size_k * 2) + (matrix_size_m + matrix_size_n)) / (latency_hmma / 1e3))/(1e12)};
            std::cout << std::fixed << "HMMA TFLOPS: " << tflops_hmma << std::endl;
        }
    }
    ...
    for (bool is_A_transpose : {true, false})
    {
        for (bool is_B_transpose : {true, false})
        {
            ...
            // Calculate TFLOPS
            double const tflops_imma{(((matrix_size_m * matrix_size_n * matrix_size_k * 2) + (matrix_size_m + matrix_size_n)) / (latency_imma / 1e3))/(1e12)};
            std::cout << std::fixed << "IMMA TFLOPS: " << tflops_imma << std::endl;
        }
    }
